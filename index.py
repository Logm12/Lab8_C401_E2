    ...
    for line in lines:
        if not header_done:
            ...
            if line.startswith("Source:"):
                ...
                # Giữ lại các dòng khác (Tiêu đề, Ghi chú) làm nội dung văn bản
                content_lines.append(line)
        else:
            content_lines.append(line)
...
def _split_by_size(
    ...
    """
    Helper: Split text dài thành chunks dựa trên đoạn văn (\n\n) để giữ ngữ cảnh.
    """
    ...
    for para in paragraphs:
        # Nếu thêm paragraph này vào mà vượt quá chunk_chars
        if current_length + len(para) > chunk_chars and current_chunk_parts:
            # Lưu chunk hiện tại
            ...
            # Tạo overlap: Giữ lại đoạn paragraph cuối cùng làm context cho chunk sau
            # (Hoặc lùi lại nếu paragraph quá dài, nhưng ở đây ưu tiên ranh giới đoạn)
            last_para = current_chunk_parts[-1]
            if len(last_para) < overlap_chars:
                current_chunk_parts = [last_para, para]
                current_length = len(last_para) + len(para) + 2 # +2 for \n\n
            else:
                current_chunk_parts = [para]
                current_length = len(para)
        else:
            current_chunk_parts.append(para)
            current_length += len(para) + (2 if len(current_chunk_parts) > 1 else 0)
...
def get_embedding(text: str) -> List[float]:
    ...
    Sử dụng model text-embedding-3-small (1536 dims).
    """
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    # Xóa collection cũ để rebuild sạch (nếu có)
    ...
    # Duyệt qua từng file với progress bar
    for filepath in tqdm(doc_files, desc="Đang index tài liệu"):
        ...
        # Step 3: Embed và lưu từng chunk vào ChromaDB
        for i, chunk in enumerate(chunks):
            # Tạo ID duy nhất cho mỗi chunk: [filename]_[chunk_index]
    ...
    print(f"\nHoàn thành! Đã index tổng cộng {total_chunks} chunks từ {len(doc_files)} tài liệu.")
